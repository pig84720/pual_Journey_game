...
import type { Step, CardState, VerseRef } from '../../types';
...
export function CityCard({ step, state = 'idle', isDraggable = true, className = '' }: CityCardProps) {
  const [isExpanded, setIsExpanded] = useState(false);

  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({
    id: `city-${step.id}`,
    data: { type: 'city', cityKey: step.id },
  ...
  return (
    <motion.div
      ...
      style={style}
      className={`
        relative bg-white rounded-card shadow-card border-2 border-brand-100
        p-4
    ...
    >
      ...
      >
        ...
        {/* 城市名稱 */}
        <h3 className="font-serif text-h3 text-brand-700 font-semibold leading-tight">
          {step.place}
        </h3>
      ...
      {isExpanded && step.note && (
        <motion.div
          ...
          className="text-body text-on/80 pt-2 border-t border-brand-100 mt-2"
        >
          {step.note}
        </motion.div>
      )}
